...
> 工作实例:
  ① 前端服务部署在在 dashboard.swain.com,服务端部署在 api.swain.com,前端请求后端，后端返回头中，携带的`set-cookie`中，`domain`字段为 `.dashboard.swain.com`，会得到一个`"this. set-cookie domain attribute was invalid with regards to the current host URL".`的错误。这就是一个跨子域设置cookie的实例，正确的方式应该设置`domain`为`.swain.com`
