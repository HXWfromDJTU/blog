...
1️⃣  服务端会将准备用于通信加密的公钥(A)送到权威证书机构(CA)，CA会用一个证书将这把送过来的公钥(A)使用非对称私钥(B')进行加密。          
...
双方都使用之前生成的(random1 + random2 + pre-master)生成了一个独一无二的公钥，接下来准备开始，双方使用独一无二的加密套件，进行测试加密传输。
